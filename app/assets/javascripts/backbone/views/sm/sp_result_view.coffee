class window.SmartMerchant.SpResultView extends SmartMerchant.View
	id: "sp_result"
	template: JST['mp/sp_result']
	model: new SpResult()
	begin_date: ""
	end_date: ""
	events:
	...
	render_summary: (json) ->	
		...
		if json.chart_data.attention.clicked_count
			$("#chart2",@$el).highcharts @setup_chart2(json.chart_data.attention,bg_date,["回复","点击"],"关注次数分布",["回复短信次数","点击链接次数"])
		else
	...
	setup_chart: (data,bg_date,tip_name,title,per = "次") ->
		chart_params = 
			...
			series:[
				...
				pointStart: Date.UTC(parseInt(bg_date[0]), parseInt(bg_date[1]) - 1, parseInt(bg_date[2]))
				data: data
			]
		return chart_params
